...
def make_safegraph(sim):
    ...
    s_days = sc.dcp(days)
    assert s_days[3] == 49 # Just after school closures -- confirm that data line up
    s_days[3] = 45 # Day schools actually closed
    s = np.interp(i_days, s_days, s)
...
def create_sim(pars=None, use_safegraph=True, label=None, show_intervs=False, people=None):
